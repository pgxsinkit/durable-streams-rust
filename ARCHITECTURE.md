...
- [Expiration and retirement](#expiration-and-retirement)
...
`Stream-TTL` and `Stream-Expires-At` share one strict, overflow-safe deadline calculation. Request-time lookup remains the protocol correctness boundary in every reaper mode. A separate B-tree index holds weak references to only expiring stream incarnations, keyed by immutable stream ID; a bounded round-robin scanner can therefore discover abandoned expired streams without walking the complete registry or updating a global heap on every TTL touch.

Retirement is coordinated with the full append acknowledgment path. An append takes a transient guard while holding the per-stream appender lock and retains it through WAL durability, visibility publication, sidecar updates, subscription notification, and its final 2xx decision. Retirement fences new guards, waits for existing guards to drain, wakes live readers, durably applies the old incarnation's subscription transition, and only then makes the path reusable. Registry and inventory removal use stream identity, so delayed work for an old incarnation cannot remove a replacement. Creation uses a bounded path-striped mutex to serialize the same path without holding a `DashMap` shard across I/O. The new state stays unpublished until its sidecar transaction is durable, then its metadata barrier covers the short registry, inventory, and expiration-index publication so retirement cannot pass between those projections; a failed fork create therefore cannot acknowledge a concurrent append and then erase it during rollback. A fork reservation updates only the parent's refcount in its existing durable sidecar, rather than capturing speculative close/dedupe/TTL fields from an append still waiting for WAL durability.

Deferred metadata and WAL checkpoint work use the same retirement fences. Hard retirement excludes sidecar writers through the stream metadata lock, unlinks one physical stream, forgets it from its WAL shard under checkpoint maintenance exclusion, and only then removes its exact registry, inventory, and expiration-index entries; the next checkpoint durably rewrites the pruned tail map. A fork parent with live children is soft-deleted and retains its bytes and WAL bookkeeping until the final child releases it. Each newly owned parent is a paced continuation retaining the original request completion and admission permit; if that exact parent already belongs to active or quarantined work, ownership transfers to it and the completed child releases its admission immediately.

Proactive deletion is gated by startup grace, a complete observe-only pass, a sticky bulk-fraction guard (only after at least 64 due streams), and a wall-versus-monotonic clock-drift guard. The startup baseline and both sticky latches live above supervised scanner generations. Scan rate, retirement admission, queue capacity, blocking work, and concurrency are bounded independently. `off` disables only proactive expiration scanning, `observe` performs no proactive deletion, and `delete` runs the same retirement coordinator used by lazy expiry and explicit `DELETE`; pageable recovery cleanup of abandoned tombstones uses that coordinator in every mode. A lossless recovery pass hands retry ownership to the coordinator instead of blocking live scans. Six failed attempts quarantine only that exact incarnation until restart and release its permit; admin status exposes the quarantine count. When a lazy-expiry GET has already admitted an exact stream incarnation, a recreate PUT joins its sticky coordinator result for at most five seconds and then retries creation once; it never admits duplicate destructive work or downgrades an explicit retirement. Recovery also rebuilds fork reference counts from the recovered child-to-parent graph and durably repairs stale zero-reference tombstones before bounded cleanup, closing the crash window between a child's durable unlink and its parent's refcount update. An unreadable sidecar is parked as `.meta.corrupt`; every later boot recognizes that marker, preserves its paired data, reserves its filename ID, and keeps fork refcount reconciliation conservative until an operator repairs it. Before WAL replay mutates any file, a read-only preflight refuses recovery when that quarantine still owns retained append or checkpoint-tail evidence; unmappable quarantine filenames fail closed, and reset never destroys the repair proof.
